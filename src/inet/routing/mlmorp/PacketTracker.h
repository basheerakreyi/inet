...
struct PacketInfo {
    ...
    L3Address forwardedTo;          // Next hop we forwarded to (chosen neighbor)
    ...
    // Default constructor
    PacketInfo() : treeId(0), forwardTime(SIMTIME_ZERO), timeout(SIMTIME_ZERO), energyBefore(0.0), energyAfter(0.0), deliveryTime(SIMTIME_ZERO), confirmed(false) {}
    
    // Parameterized constructor
    PacketInfo(int tid, const L3Address& src, const L3Address& dst, 
               const L3Address& fwd, simtime_t time, simtime_t to,
               const std::vector<double>& nf, double energy)
        : treeId(tid), source(src), destination(dst), forwardedTo(fwd),
          forwardTime(time), timeout(to), neighborFeatures(nf), 
    ...
    PacketInfo(const PacketInfo& other)
        : treeId(other.treeId), source(other.source), destination(other.destination),
          ...
          timeout(other.timeout), neighborFeatures(other.neighborFeatures),
          energyBefore(other.energyBefore), energyAfter(other.energyAfter),
          deliveryTime(other.deliveryTime), confirmed(other.confirmed) {}
    ...
    PacketInfo& operator=(const PacketInfo& other) {
        if (this != &other) {
            ...
            neighborFeatures = other.neighborFeatures;
